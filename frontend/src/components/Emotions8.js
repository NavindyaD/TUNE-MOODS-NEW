...
function Emotions() {
    ...
    // Add disgust-specific playlist here
    const disgustPlaylist = [
        "https://www.youtube.com/watch?v=videoID1", // Replace with actual YouTube video IDs
    ...
    return (
        <div className="emotions-app-container">
            ...
            {emotion && emotion.toLowerCase() === "disgust" && (
                <>
                    <h3>Disgust Playlist</h3>
                    {disgustPlaylist.map((videoUrl, index) => (
                        <YouTube
                            key={index}
